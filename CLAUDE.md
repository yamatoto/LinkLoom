...
1. すべての実装が完了している
2. すべてのテストが通っている
3. 静的解析（lint, tsc）がエラーゼロ
4. Chrome DevTools MCPで動作確認済み
5. **コードレビューで指摘されたすべての事項（Warning、Suggestion含む）が対応済み**

**🚧 実装完了・改善TODO残（85-95%）**:
- 機能は動作するが、コードレビューで指摘された改善点が残っている
- 例: Link化、統合テスト追加、パフォーマンス最適化など
- **このステータスでは「完了」と表記しない**
- **このステータスでは次の機能実装に進まない**

**❌ 完了と判断してはいけない例**:
- コードレビューでCritical、Warning、Suggestionが残っている（**優先度に関わらず**）
- E2Eテストが実行できていない（ハング問題など）
- 統合テストが未実装
- 「動くけど品質に課題がある」状態

**重要な方針**:
- **レビュー指摘事項は優先度（Critical/Warning/Suggestion）に関わらず、すべて対応完了してから次の機能に進む**
- 「優先度: 中」「優先度: 低」であっても、TODOとして残さず即座に対応する
- 品質を担保してから次のステップへ進むことで、技術的負債の蓄積を防ぐ
...
2. **ユーザーの許可が得られたら**
   ...
   - **TODOが残っている場合**: 🚧 マーク + 進捗率（85-95%）を記載
   - **すべて完了している場合**: ✅ マーク + 100%を記載
   - 進捗バーを更新（該当する場合）
   - **最終更新日**を更新
